...
| Lifecycle event outbox | `<persisted_sandbox_store_path>/lifecycle-events.db` | Synchronous RocksDB log with a persistent stream UUID, monotonic sequence, acknowledgement high-water mark, and ordered event records | Retries create/fork/pause/resume/delete control-plane updates across scheduler outages and envd restarts | Event and next sequence are one sync write batch. Acknowledgement and record deletion are one sync write batch. Startup fails closed on gaps, corrupt records, or an unsupported schema version. |
