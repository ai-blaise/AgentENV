...
- A C toolchain and `libclang`. The ublk bindings are generated by `bindgen` at
  build time, which loads `libclang` at runtime rather than linking it, so a
  host without it fails in the `libublk-rs-sys` build script rather than at
  configure time:
  ...
  If `libclang` is installed somewhere the default search does not reach, set
  `LIBCLANG_PATH` to the directory holding it.
- `sudo` access
