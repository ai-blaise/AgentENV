...
- Scheduler can run as a single primary, as N full replicas behind one round-robin address sharing a binding store and replicating node state to each other, or as query-only replicas that serve only `LookupNode` from Redis.
...
- `--mode` selects how this scheduler takes part in a cluster: `primary` (the default), `replica`, or `query-only`. `--query-only` is kept as a deprecated alias for `--mode=query-only`; setting both is refused. `SCHEDULER_MODE=<mode>` sets it from the environment when the flag is absent.
  - `primary` is the single-scheduler deployment. It is the only mode that may run with `scheduler.redis_addr` empty, and it says so once at startup: bindings that live in one process are lost on restart and cannot be shared.
  - `replica` is one of N schedulers behind one address, sharing a binding store and replicating node state to each other. It refuses to start without `scheduler.redis_addr`, because N replicas with N private binding maps answer a routing lookup correctly one time in N and nothing says so until a client is told a running sandbox does not exist.
  - `query-only` starts a read-only scheduler that supports only `LookupNode`; it requires `scheduler.redis_addr` and needs no node discovery config.
- `scheduler.node_stream_enabled` replicates node liveness and capacity between schedulers over Redis Streams. It defaults to on for `--mode=replica` and off everywhere else; `false` is the rollback and reproduces a scheduler that knows only the nodes that heartbeat to it.

  A node's gRPC connection is sticky — kube-proxy binds it to one scheduler for the life of the TCP connection — so with N replicas each hears from roughly 1/N of the fleet. Without replication that is worse than one scheduler: each replica places all of its traffic onto its own slice of the fleet, computes the cluster-wide CPU intersection over that slice alone, and a freshly started replica places over an empty registry. The replica that received a heartbeat republishes it, stripped of everything that belongs to it alone: the roster and its digest stay with that replica, so the stream carries O(nodes) rather than O(sandboxes). Nodes are unchanged and never speak to Redis.

  Sixteen shards, hashed from the node id, are read by every replica with a plain `XREAD` — each replica needs every message rather than a share of them. Convergence is `agentenv_scheduler_node_stream_lag_seconds`; `agentenv_scheduler_registry_nodes{source="rpc"|"stream"}` splits each replica's registry by how it learned of each node, and on a converged fleet the two sum to the fleet size on every replica. `agentenv_scheduler_node_stream_applied_total{outcome}` counts each event once, and `agentenv_scheduler_node_stream_clamped_total` counts those whose freshness stamp had to be pulled back into the reading replica's clock — a steady rate there is replica clocks that disagree, which makes every freshness decision wrong by that much.
- `scheduler.node_stream_maxlen` (default `100000`) bounds each shard. It has to hold more than two `scheduler.report_ttl` of the whole fleet's heartbeats, or a replica that restarts replays a window with live nodes missing from it and `agentenv_scheduler_node_stream_warmup_incomplete` goes to 1.
- `scheduler.node_stream_publish_queue` (default `4096`) bounds the events waiting to be written when Redis is slow. Publishing never blocks a heartbeat, so a full queue drops instead and counts `agentenv_scheduler_node_stream_dropped_total{reason="queue_full"}`; the cost is one interval of staleness on the other replicas, because the node's next heartbeat republishes its whole state.
- `scheduler.node_stream_warmup_timeout` (default `15s`) bounds how long a starting replica reports `NOT_SERVING` while it replays the fleet. Waiting indefinitely would let a Redis blip during a rollout take the whole scheduler tier out; serving early leaves a partial registry, which is what a single scheduler does after every restart.
- `scheduler.store_probe_interval` (default `2s`) is how often a scheduler asks its binding store whether it is reachable. Three consecutive failures report the `scheduler.v1.Scheduler` health service as `NOT_SERVING`, one success restores it, and `agentenv_scheduler_store_reachable` is the gauge. This is load-bearing under replicas: gRPC's round-robin balancer drops a subchannel that cannot connect, not one that answers with an error, so without the probe a replica that has lost Redis keeps taking its share of routing lookups and failing all of them indefinitely.
...
- `SCHEDULER_MODE=<primary|replica|query-only>` sets the mode when `--mode` is not passed. `SCHEDULER_NODE_STREAM_ENABLED=<bool>`, `SCHEDULER_NODE_STREAM_WARMUP_TIMEOUT=<duration>` and `SCHEDULER_STORE_PROBE_INTERVAL=<duration>` override the corresponding keys.
...
- `gateway.scheduler_addr` points to the primary scheduler. The gateway uses it for scheduling, assignment writes, node listing, node detail resolution, and P2P scheduler APIs. It accepts one address or a comma-separated list (`sched-1:9090,sched-2:9090,sched-3:9090`), and both forms balance with round-robin across every address the name resolves to. In Kubernetes point it at a headless service so DNS returns the replica addresses themselves; a ClusterIP resolves to one virtual address, which pins each gateway to whichever replica kube-proxy picked. The comma-separated form is for deployments with no cluster DNS to ask.
...
- For full scale-out, run N schedulers with `--mode=replica` and the same `scheduler.redis_addr`, behind a headless Service, and point `gateway.scheduler_addr` at that name so RPCs round-robin over them. Each replica takes the heartbeats of the nodes whose connections landed on it and replicates their liveness and capacity to the others, so every replica places over the whole fleet and computes the same CPU intersection. A replica reports the `scheduler.v1.Scheduler` health service as `NOT_SERVING` until it has replayed the fleet and its binding store has answered, which is what keeps a starting or Redis-partitioned replica out of the gateway's rotation. Two deployment details make that effective: the readiness probe must name the service (`/grpc_health_probe -addr=127.0.0.1:9090 -service=scheduler.v1.Scheduler`), and the headless Service must not set `publishNotReadyAddresses`. The liveness probe stays on the overall status, which is SERVING for as long as the process is: restarting a scheduler because Redis is unreachable turns an outage into a crash loop across the whole tier.
- For data-plane-only high availability, run one primary scheduler with `scheduler.redis_addr` set and multiple query-only scheduler replicas started with `--mode=query-only` against the same Redis. Point gateways at the primary with `gateway.scheduler_addr` and at the query-only service with `gateway.query_only_scheduler_addr`. The primary writes sandbox bindings to Redis while query-only replicas continue to serve data-plane `LookupNode` during primary restarts or upgrades. This HA mode is intentionally data-plane only: requests that proxy to existing sandboxes can keep routing, but control-plane operations that need the primary scheduler, such as creating new sandboxes, scheduling, assignment writes, node listing, node detail resolution, and P2P scheduler APIs, still fail while the primary scheduler is unavailable. Artifact store state is still in-memory and is not covered by this HA mode.
- The gateway is intentionally left as ClusterIP by default; attach an Ingress or LoadBalancer based on your environment.

## gRPC API

Proto contract: api/proto/scheduler.proto. Messages exchanged between scheduler replicas rather than with nodes live in api/proto/node_stream.proto, which is deliberately outside the contract the Rust node client is generated from.
